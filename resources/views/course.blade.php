				...
				<div class="collapse navbar-collapse" id="navbarTogglerDemo02">
					<ul class="navbar-nav ml-auto">
						<li class="nav-item @@home__active">
							<a class="nav-link" href="{{url('/index1')}}">Home <span class="sr-only">(current)</span></a>
						</li>
...
<script>
  ...
  <!--theme switcher dark and light mode script//-->
    ...
    <div class="container mt-5">
        <h1 class="text-center" style="color:white;">Course</h1> <br><br>
                ...
                <table class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" style="color:white;">Coursename</th>
                    ...
                    <tbody>
                        @foreach ($course as $key => $course)
                            <tr>
                                <td style="color:pink;font-size:20px;">{{ $loop->iteration}}</td>
                                <td style="color:pink;font-size:20px;">{{ $course->Coursename }}</td>
                                <td style="color:pink;font-size:20px;">{{ $course->Duration }}</td>
                                <td style="color:pink;font-size:20px;">{{ $course->Time }}</td>
                                <td style="color:pink;font-size:20px;">{{ $course->Instructor }}</td>
                            
                                <td style="font-size:20px;">
                                    @if ($course->Status == 1)
                                        <button class="btn btn-primary" onclick="openClassLink('{{ $course->Link }}')">Class Link</button>
								...
								<td><a href="{{ url('/deletecourse/'.$course->Id) }}" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this course?')">Delete Course</a></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
